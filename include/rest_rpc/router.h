...
private:
  ...
  static auto call_helper(const F &f, const std::index_sequence<I...> &,
  ...
  static auto call(const F &f, std::weak_ptr<connection> ptr,
                   std::string &result, std::tuple<Arg, Args...> tp) {
    if constexpr (std::is_void_v<typename std::invoke_result_t<
                      F, std::weak_ptr<connection>, Args...>>) {
  ...
  template <typename F, typename Self, size_t... Indexes, typename Arg,
            typename... Args>
  static auto call_member_helper(
  ...
  static auto call_member(const F &f, Self *self, std::weak_ptr<connection> ptr,
                          std::string &result, std::tuple<Arg, Args...> tp) {
    if constexpr (std::is_void_v<std::invoke_result_t<
                      F, Self, std::weak_ptr<connection>, Args...>>) {
